...
// ファイル選択の処理
gifFileInput.addEventListener('change', async (event) => {
    const file = event.target.files[0]
    if (!file) return

    try {
        info.textContent = 'GIF を読み込んでいます...'
        ...
        info.textContent = `${gifFrames.length} フレームを読み込みました`
        ...
        info.textContent = `エラー: ${error.message}`
    }
})
...
speedSlider.addEventListener('input', (event) => {
    ...
    speedValue.textContent = playbackSpeed.toFixed(1)
})
